...
function parseExcel(buffer: ArrayBuffer): ParsedCaixaRow[] {
...
export default function CaixaRespuestasPage() {
  ...
  // Filter rows by date for preview count
  const filteredCount = parsedRows.filter((r) => {
    if (!r.col_I) return true
    const d = new Date(r.col_I)
    return isNaN(d.getTime()) || d >= new Date(dateFrom)
  }).length
